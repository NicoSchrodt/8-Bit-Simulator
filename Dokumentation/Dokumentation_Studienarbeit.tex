...
Mit diesem Simulator soll es möglich sein Assembler Code in ein Textfeld einzugeben und diesen dann ausführen zu können. Damit dies möglich ist muss der Assembler Code erst compiliert werden. Dies wird mit einem einfachen Compiler aus dem Internet gemacht. Dieser erstellt aus dem Source Code (.asm) eine Compilierte Version (.com), wobei die Befehle Byteweise ausgelesen werden können. Manche Befehle verwenden aber auch mehr als ein Byte. Zusätzlich dazu wird noch eine Datei (.sym) erstellt, die die Lables/Sprungmarken abspeichert.
